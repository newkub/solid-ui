...
export const createCommandRepository = (
	...
	save: (command) => {
		...
		return Promise.resolve(
			result.success
				? { success: true, data: result.data.commands.get(command.id)! }
				: result,
		);
	},
	...
	update: (id, updates) => {
		...
		return Promise.resolve(
			result.success
				? { success: true, data: result.data.commands.get(id)! }
				: result,
		);
	},
	delete: (id) => {
	...
	findByCategory: (category) =>
		Promise.resolve(findByCategory(state, category)),
	...
	saveMany: (commands) => {
		...
		return Promise.resolve(
			result.success ? { success: true, data: commands } : result,
		);
	},
	deleteMany: (ids) => {
	...
	countByCategory: (category) =>
		Promise.resolve(countByCategory(state, category)),
